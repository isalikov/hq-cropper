...
function getResultImageSquare(css = { }, state) {
    const node = document.createElement('img')
    ...
    applyClassNames(node, css.resultImageSquare)
...
function getResultContainer(css = { }, state) {
    const node = document.createElement('div')

    applyClassNames(node, css.resultContainer)
    ...
    node.appendChild(
        getResultImageSquare(css, state),
    )
